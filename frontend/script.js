    ...
    // Macro save mini-toast
    $('#saveMacros').addEventListener('click',()=>{ const msg=$('#macroMsg'); msg.style.color='var(--ok)'; msg.textContent=(LANG==='hu')?'✅ Mentve (demo)':'✅ Saved (demo)'; setTimeout(()=>{ msg.style.color='var(--muted)'; msg.textContent=''; },1800); });
    ...
    function renderFood(list){ const box=$('#foodResults'); box.innerHTML=''; if(!list.length){ box.innerHTML=`<div style="padding:10px;color:var(--muted)">${LANG==='hu'?'Nincs találat.':'No results.'}</div>`; return; }
    ...
    function doFoodSearch(){ const q=($('#foodQuery').value||'').toLowerCase().trim(); const res=q?FOOD.filter(x=>x.n.toLowerCase().includes(q)):FOOD; renderFood(res); }
    $('#foodSearch').addEventListener('click',doFoodSearch); $('#foodQuery').addEventListener('keydown',e=>{ if(e.key==='Enter') doFoodSearch(); }); doFoodSearch();
    ...
    function buildDietTabs(){ const tabs=$('#dietTabs'); tabs.innerHTML=''; DAYS_KEYS.forEach((k,i)=>{ const b=document.createElement('button'); b.className='tab'+(i===activeDayIndex?' active':''); b.textContent=dayName(k); b.addEventListener('click',()=>{ activeDayIndex=i; buildDietTabs(); renderDayMeals(); }); tabs.appendChild(b); }); }
    function renderDayMeals(){ const box=$('#dietDayList'); const items=WEEK[activeDayIndex]; box.innerHTML=''; if(!items.length){ const empty=document.createElement('div'); empty.style.cssText='padding:12px 20px;color:var(--muted)'; empty.textContent=(LANG==='hu')?'Nincs még tétel ezen a napon.':'No items yet for this day.'; box.appendChild(empty); return; } items.forEach(obj=>{ const row=document.createElement('div'); row.className='meal-item'; row.innerHTML=`<span>${obj.name}</span><span class="kcal">${obj.kcal} kcal</span>`; box.appendChild(row); }); }
    function addMealToDay(i,item){ WEEK[i].push(item); renderDayMeals(); }
    buildDietTabs(); renderDayMeals();

    // Drag & Drop (demo)
    const dragArea=$('#dragArea'); dragArea.addEventListener('dragover',e=>{e.preventDefault(); dragArea.style.borderColor='var(--gold-1)';});
    ...
    function t(){ const S=TXT[LANG];
      $('#navDiet').textContent=S.navDiet; $('#navWorkout').textContent=S.navWorkout; $('#navSignin').textContent=S.navSignin; $('#getStarted').textContent=S.cta;
      $('#homeTitle').textContent=S.homeTitle; $('#homeSubtitle').textContent=S.homeSubtitle; $('#goDiet').textContent=S.goDiet; $('#goWorkout').textContent=S.goWorkout;
      $('#dietTitle').textContent=S.dietTitle; $('#dietSubtitle').textContent=S.dietSubtitle; $('#macroTitle').textContent=S.macroTitle; $('#calLabel').textContent=S.calLabel; $('#foodSearchTitle').textContent=S.foodSearchTitle; $('#weeklyDietTitle').textContent=S.weeklyDietTitle;
      $('#bmiTitle').textContent=S.bmiTitle; $('#tipTitle').textContent=S.tipTitle; $('#tipCopy').textContent=S.tipCopy; $('#qsTitle').textContent=S.qsTitle; $('#qsCopy').textContent=S.qsCopy;
      $('#workoutTitle').textContent=S.workoutTitle; $('#workoutSubtitle').textContent=S.workoutSubtitle; $('#weekPlanTitle').textContent=S.weekPlanTitle; $('#dMon').textContent=S.dMon; $('#dWed').textContent=S.dWed; $('#dFri').textContent=S.dFri;
      buildDietTabs(); renderDayMeals(); doFoodSearch();
    }
    $('#langToggle').addEventListener('click',()=>{ LANG=(LANG==='hu')?'en':'hu'; t(); });
    t();
    ...
    body: JSON.stringify({email:'demo@athlion.app', password:'123456', user_name:'Demo'})
    });
